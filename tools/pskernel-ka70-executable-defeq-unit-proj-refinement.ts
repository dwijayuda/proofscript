...
const CHECKPOINT = 'proofscript-v1-ka70-executable-defeq-unit-proj-refinement0';
const VERSION = '1.0.0-pskernel.73';
const BASELINE = 'proofscript-v1-ka69-executable-defeq-delta-string-refinement0';
const CORE_FORMAT = 71;
const CERT_FORMAT = 2;
const BASELINE_OBLIGATIONS = 212;
...
const relLean = 'assurance/ka70/executable-defeq-unit-proj-refinement-bridge.lean';
const destLean = 'PSKernelKA70ExecutableDefEqUnitProjRefinementBridge.lean';
const counted = [
...
const CHECKPOINT_ZIP = '/mnt/data/proofscript-v1-ka69-executable-defeq-delta-string-refinement0.zip';
const CHECKPOINT_SHA = '/mnt/data/proofscript-v1-ka69-executable-defeq-delta-string-refinement0.zip.sha256';
const OLD_CODEBASE_POLICY = {
  oldCodebaseRegressionIgnoredByUser: true,
  fullNpmTestRequiredForThisCheckpoint: false,
  reason: 'User requested continuation on the strict Lean4Lean KA lineage. KA70 changes only Lean4Lean bridge wrappers and release metadata; it does not touch trusted PSKernel semantics.'
};
...
function baselineArtifactAudit() {
  ...
  try { internalReleaseGateSha = readJson('assurance/ka69/KA69_EXECUTABLE_DEFEQ_DELTA_STRING_REFINEMENT_RELEASE_GATE.json').finalZipSha256 ?? null; } catch {}
  try { internalVerificationSha = readJson('assurance/ka69/KA69_EXECUTABLE_DEFEQ_DELTA_STRING_REFINEMENT_VERIFICATION_SUMMARY.json').finalZipSha256 ?? null; } catch {}
  return {
    ...
    policy: 'External .sha256 file is authoritative for ZIP integrity; KA70 avoids embedding a self ZIP hash inside the ZIP because that creates stale self-reference risk.'
  };
}
...
function architectureHealth() {
  ...
  const scan = (dir: string) => {
    ...
    for (const ent of fs.readdirSync(path.join(root, dir), { withFileTypes: true })) {
      ...
      if (!ent.isFile() || !/ka70|KA70/.test(rel) || !/\.(ts|lean|md|json)$/.test(ent.name)) continue;
  ...
  return { antiSpaghettiGatePassed: oversized.length === 0 && toolFiles.length <= 2 && !semanticPackageTouched, ka70Files: files, ka70ToolFiles: toolFiles, newKA70OversizedFiles: oversized, semanticPackageTouched, forbiddenPrefixes };
}
...
function checkLean() {
  ...
  if (!exists(relLean)) return { status: 'failed', reason: 'ka70_lean_module_missing', modules: [] as any[] };
  const build = run(lakePath, ['build', 'Lean4Lean.Verify.TypeChecker.IsDefEq'], lean4leanRoot);
  if (build.status !== 0) return { status: 'failed', reason: 'lean4lean_isdefeq_build_failed', modules: [{ ka: 'KA70-dependency-build', status: 'failed', proofBearing: false, obligations: 0, stdoutTail: (build.stdout ?? '').slice(-1800), stderrTail: (build.stderr ?? '').slice(-1800) }] };
...
function progress() {
  return {
    ...
    metricPolicy: 'KA70 advances the exact formal obligation counter only; percentage estimates remain held pending a separate coverage audit.',
    notAFormalEquivalenceClaim: true
  };
}

function writeReports(result: any) {
  writeJson('assurance/ka70/KA70_EXECUTABLE_DEFEQ_UNIT_PROJ_REFINEMENT_RELEASE_GATE.json', result.releaseGate);
  writeJson('assurance/ka70/KA70_EXECUTABLE_DEFEQ_UNIT_PROJ_REFINEMENT_VERIFICATION_SUMMARY.json', result.verificationSummary);
  writeJson('assurance/ka70/KA70_KERNEL_FEATURE_EQUIVALENCE_PROGRESS.json', result.featureEquivalenceProgress);
  writeJson('assurance/ka70/KA70_EXECUTABLE_DEFEQ_UNIT_PROJ_REFINEMENT_BRIDGE_SPEC.json', result.bridgeSpec);
  fs.writeFileSync(path.join(root, 'assurance/ka70/KA70_KERNEL_FEATURE_EQUIVALENCE_PROGRESS_REPORT.md'), `# KA-70 Kernel Feature Equivalence Progress\n\nCheckpoint: \`${CHECKPOINT}\`\n\nPublic version: \`${VERSION}\`\n\n- Feature-surface bridge progress: **${FEATURE_PROGRESS}%** (held pending audit)\n- Executable-kernel equivalence proof progress: **${EXECUTABLE_PROGRESS}%** (held pending audit)\n- Arena corpus regression evidence: **100%**\n- Formal Lean4Lean bridge obligations: **${TOTAL_OBLIGATIONS}**\n\nKA-70 adds four strict DefEq unit/proj bridge obligations. Percentage estimates are intentionally held without a separate coverage audit.\n`);
  fs.writeFileSync(path.join(root, 'assurance/ka70/KA70_EXECUTABLE_DEFEQ_UNIT_PROJ_REFINEMENT_REPORT.md'), `# KA-70 Executable DefEq Unit/Proj Refinement Report\n\nCheckpoint: \`${CHECKPOINT}\`\n\nPublic version: \`${VERSION}\`\n\nBaseline: \`${BASELINE}\`\n\n## What changed\n\nKA-70 continues from KA-69 and adds four strict Lean4Lean DefEq bridge wrappers for string literal core expansion, unit-like shortcut, projected lazy-delta finish, and projected lazy-delta loop. No trusted PSKernel semantics, codec, Core format, or certificate format are changed.\n\n## Machine-checked bridge lemmas\n\n${counted.map(x => `- \`${x}\``).join('\n')}\n\n## Baseline artifact audit\n\n- KA-69 external SHA file matches actual KA-69 ZIP: **${result.baselineArtifactAudit.externalShaMatchesActual}**\n- KA-69 internal finalZipSha256 fields known stale: **${result.baselineArtifactAudit.internalSelfHashFieldsKnownStale}**\n- KA-70 policy: external .sha256 file is authoritative; self ZIP hash is not embedded inside the ZIP.\n\n## No-spaghetti result\n\n- Anti-spaghetti gate: **${result.architectureHealth.antiSpaghettiGatePassed ? 'passed' : 'failed'}**\n- KA-70 tool files: **${result.architectureHealth.ka70ToolFiles.length}**\n- New KA-70 oversized files: **${result.architectureHealth.newKA70OversizedFiles.length}**\n- Semantic package touched: **${result.architectureHealth.semanticPackageTouched}**\n\n## Boundary\n\n- Full Lean4 equivalence: **no**\n- Same theory as full Lean4: **no**\n- Fully formal K3: **no**\n- Executable PSKernel refinement proof: **no**\n- Full executable WHNF/DefEq refinement: **no**\n- Trusted PSKernel semantic change: **no**\n- Core format changed: **no**\n- Certificate format changed: **no**\n`);
}

export function runKA70(options: { strict?: boolean; writeReports?: boolean } = {}) {
  const pkg = readJson('package.json');
  assert.equal(pkg.version, VERSION);
  const baselineProgress = readJson('assurance/ka69/KA69_KERNEL_FEATURE_EQUIVALENCE_PROGRESS.json');
  assert.equal(baselineProgress.formalLean4LeanBridgeObligations, BASELINE_OBLIGATIONS, 'KA69 progress JSON must support the baseline');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const result = runKA70({ strict: process.argv.includes('--strict'), writeReports: true });
