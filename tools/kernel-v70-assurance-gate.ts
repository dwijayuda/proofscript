...
if (versions.kernelArtifactFormat !== 70) fail('versions.json is not Core format 70');
if (versions.implementationProfile !== 'KERNEL-projection-conformance1') fail('wrong v70 implementation profile');
...
pass('Core v70 / KERNEL-projection-conformance1 / Lean 4.33.1 identity');
...
const universeGate=run(process.execPath,[path.join(root,'tools/kernel-universe-conformance1-tests.ts')]);
if(universeGate.status!==0) fail(`inherited v69 universe gate failed\n${universeGate.stdout}\n${universeGate.stderr}`);
pass('inherited v69 universe conformance regression');
const identityGate=run(process.execPath,[path.join(root,'tools/kernel-projection-v70-identity-tests.ts')]);
if(identityGate.status!==0) fail(`v70 identity/codec gate failed\n${identityGate.stdout}\n${identityGate.stderr}`);
pass('v70 projection profile identity/codec regression');
const projectionGate=run(process.execPath,[path.join(root,'tools/kernel-projection-conformance1-tests.ts')]);
if(projectionGate.status!==0) fail(`v70 projection semantic gate failed\n${projectionGate.stdout}\n${projectionGate.stderr}`);
pass('v70 exact projection dependency/Prop-gating regression');
...
const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'ps-v70-formal-'));
try {
  const outDir=path.join(tmp,'ProofScriptKernelEquivalence'); fs.mkdirSync(outDir,{recursive:true});
  const modules=['LevelSemantics','LevelLeanCorrespondence','ExprTranslation','ExprOperations','TypingDirect','ReductionBetaZeta','DeltaTransparency','ReductionOrdinary','DefEqOrdinary','TypingConversion','Projection','RecursorMetadata','RecursorDirect','Quotient'];
  let leanPath='';
  ...
  const projectionTs=fs.readFileSync(path.join(evidence,'ProjectionTSDirect.out'),'utf8');
  if(!projectionTs.includes('PROJECTION_TS_CASES=1000 FAILURES=0'))
    fail('TypeScript projection evidence is not clean');
  const projectionNative=fs.readFileSync(path.join(evidence,'ProjectionNativeDifferential.out'),'utf8');
  if(!projectionNative.includes('PROJECTION_NATIVE_CASES=1000 FAILURES=0 KERNEL_CHECKED=1000'))
    fail('exact Lean raw projection evidence is not clean');
  const oldV69Projection=fs.readFileSync(path.join(evidence,'projection-prop-dependency-old-v69.out'),'utf8');
  if(!oldV69Projection.includes('HISTORICAL_V69_ACCEPTED dependent-proof-projection'))
    fail('historical v69 projection counterexample is missing');
  const liveProjectionTs=run(process.execPath,[path.join(root,'tools/kernel-projection-differential-tests.ts')]);
  if(liveProjectionTs.status!==0 || !liveProjectionTs.stdout.includes('PROJECTION_TS_CASES=1000 FAILURES=0'))
    fail(`live TypeScript projection differential failed\n${liveProjectionTs.stdout}\n${liveProjectionTs.stderr}`);
  const liveProjectionNative=run(lean,[path.join(evidence,'ProjectionNativeDifferential.lean')]);
  if(liveProjectionNative.status!==0 || !(`${liveProjectionNative.stdout}\n${liveProjectionNative.stderr}`).includes('PROJECTION_NATIVE_CASES=1000 FAILURES=0 KERNEL_CHECKED=1000'))
    fail(`live exact Lean raw projection differential failed\n${liveProjectionNative.stdout}\n${liveProjectionNative.stderr}`);
  const exactProjection=run(lean,[path.join(evidence,'ProjectionConformanceExact.lean')]);
  if(exactProjection.status!==0 || !(`${exactProjection.stdout}\n${exactProjection.stderr}`).includes('PASS ProjectionConformanceExact: exact Lean 4.33.1 raw projection semantics'))
    fail(`exact raw projection conformance regression failed\n${exactProjection.stdout}\n${exactProjection.stderr}`);
  pass('1,000 TypeScript + 1,000 exact-Lean raw projection cases plus minimized historical-v69 counterexample');

  const recMetaNative=fs.readFileSync(path.join(evidence,'RecursorMetadataNativeDifferential.out'),'utf8');
  const recMetaTs=fs.readFileSync(path.join(evidence,'RecursorMetadataTSDirect.out'),'utf8');
  if(!recMetaNative.includes('RECURSOR_METADATA_NATIVE_CASES=7 FAILURES=0 RAW_KERNEL_DECLARATIONS=7'))
    fail('exact Lean recursor-metadata evidence is not clean');
  if(!recMetaTs.includes('RECURSOR_METADATA_TS_CASES=7 FAILURES=0'))
    fail('TypeScript recursor-metadata evidence is not clean');
  const recMetaLines=text=>text.split(/\r?\n/).filter(line=>line.startsWith('REC_META '));
  const recordedLeanMeta=recMetaLines(recMetaNative), recordedTsMeta=recMetaLines(recMetaTs);
  if(recordedLeanMeta.length!==7 || JSON.stringify(recordedLeanMeta)!==JSON.stringify(recordedTsMeta))
    fail('recorded Lean/TypeScript recursor metadata does not agree exactly');
  const liveRecMetaTs=run(process.execPath,[path.join(root,'tools/kernel-recursor-metadata-differential-tests.ts')]);
  if(liveRecMetaTs.status!==0 || !liveRecMetaTs.stdout.includes('RECURSOR_METADATA_TS_CASES=7 FAILURES=0'))
    fail(`live TypeScript recursor-metadata differential failed\n${liveRecMetaTs.stdout}\n${liveRecMetaTs.stderr}`);
  const liveRecMetaNative=run(lean,[path.join(evidence,'RecursorMetadataNativeDifferential.lean')]);
  const liveRecMetaNativeText=`${liveRecMetaNative.stdout}\n${liveRecMetaNative.stderr}`;
  if(liveRecMetaNative.status!==0 || !liveRecMetaNativeText.includes('RECURSOR_METADATA_NATIVE_CASES=7 FAILURES=0 RAW_KERNEL_DECLARATIONS=7'))
    fail(`live exact Lean recursor-metadata differential failed\n${liveRecMetaNative.stdout}\n${liveRecMetaNative.stderr}`);
  const liveLeanMeta=recMetaLines(liveRecMetaNativeText), liveTsMeta=recMetaLines(liveRecMetaTs.stdout);
  if(liveLeanMeta.length!==7 || JSON.stringify(liveLeanMeta)!==JSON.stringify(liveTsMeta))
    fail('live Lean/TypeScript recursor metadata does not agree exactly');
  pass('7 direct raw-kernel recursor metadata shapes agree exactly between TypeScript and Lean 4.33.1');

  const recK=run(process.execPath,[path.join(root,'tools/kernel-recursor-k-tests.ts')],{env:{PROOFSCRIPT_LEAN_BIN:lean}});
  if(recK.status!==0 || !recK.stdout.includes('RecursorVal.k classification'))
    fail(`recursor-K exact differential failed\n${recK.stdout}\n${recK.stderr}`);
  const recMinor=run(process.execPath,[path.join(root,'tools/kernel-recursor-minor-order-tests.ts')],{env:{PROOFSCRIPT_LEAN_BIN:lean}});
  if(recMinor.status!==0 || !recMinor.stdout.includes('fields-first recursive minor ordering'))
    fail(`recursor minor-order exact differential failed\n${recMinor.stdout}\n${recMinor.stderr}`);
  const recIndexed=run(process.execPath,[path.join(root,'tools/kernel-indexed-recursors-tests.ts')],{env:{PROOFSCRIPT_LEAN_BIN:lean}});
  if(recIndexed.status!==0 || !recIndexed.stdout.includes('direct+higher-order indexed recursors'))
    fail(`indexed-recursor exact differential failed\n${recIndexed.stdout}\n${recIndexed.stderr}`);
  pass('recursor K, fields-first iota/minor ordering, and indexed-recursion exact-Lean regressions');


  const quotTs=fs.readFileSync(path.join(evidence,'QuotientTSDirect.out'),'utf8');
  const quotNative=fs.readFileSync(path.join(evidence,'QuotientNativeDifferential.out'),'utf8');
  if(!quotTs.includes('QUOT_TS_LIFT=500 IND=500 LIFT_FAILURES=0 IND_FAILURES=0'))
    fail('TypeScript quotient evidence is not clean');
  if(!quotTs.includes('QUOT_STRUCTURAL_LINES=4 MISMATCHES=0'))
    fail('recorded quotient structural comparison is not clean');
  if(!quotNative.includes('QUOT_NATIVE_LIFT=500 IND=500 LIFT_FAILURES=0 IND_FAILURES=0 KERNEL_CHECKS=1000'))
    fail('exact Lean quotient evidence is not clean');
  const quotLines=text=>text.split(/\r?\n/).filter(line=>line.startsWith('Q|'));
  const recordedQuotTs=quotLines(quotTs), recordedQuotLean=quotLines(quotNative);
  if(recordedQuotTs.length!==4 || JSON.stringify(recordedQuotTs)!==JSON.stringify(recordedQuotLean))
    fail('recorded Lean/TypeScript quotient primitive structures do not agree exactly');
  const liveQuot=run(process.execPath,[path.join(root,'tools/kernel-quotient-differential-tests.ts')],{env:{PROOFSCRIPT_LEAN_BIN:lean}});
  if(liveQuot.status!==0 || !liveQuot.stdout.includes('QUOT_STRUCTURAL_LINES=4 MISMATCHES=0') ||
      !liveQuot.stdout.includes('QUOT_TS_LIFT=500 IND=500 LIFT_FAILURES=0 IND_FAILURES=0'))
    fail(`live quotient differential failed\n${liveQuot.stdout}\n${liveQuot.stderr}`);
  const legacyQuot=run(process.execPath,[path.join(root,'tools/kernel-quotient-tests.ts')],{env:{PROOFSCRIPT_LEAN_BIN:lean}});
  if(legacyQuot.status!==0 || !legacyQuot.stdout.includes('exact Lean 4.33.1 quotient primitive/type/computation evidence passed'))
    fail(`legacy quotient admission/computation regression failed\n${legacyQuot.stdout}\n${legacyQuot.stderr}`);
  pass('4 exact quotient primitive structures + 1,000 TypeScript/Lean quotient computation cases');
} finally {
  fs.rmSync(tmp,{recursive:true,force:true});
}

console.log('KERNEL v70 ASSURANCE GATE: PASS');
