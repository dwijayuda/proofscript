...
const CHECKPOINT = 'proofscript-v1-ka75-executable-infer-atomic-refinement0';
const VERSION = '1.0.0-pskernel.78';
const BASELINE = 'proofscript-v1-ka74-executable-defeq-nat-offset-boundary-refinement0';
const CORE_FORMAT = 71;
const CERT_FORMAT = 2;
const BASELINE_OBLIGATIONS = 221;
const NEW_OBLIGATIONS = 4;
const TOTAL_OBLIGATIONS = 225;
const FEATURE_PROGRESS = 89;
const EXECUTABLE_PROGRESS = 56;
...
const relLean = 'assurance/ka75/executable-infer-atomic-refinement-bridge.lean';
const destLean = 'PSKernelKA75ExecutableInferAtomicRefinementBridge.lean';
const counted = [
...
function architectureHealth() {
  ...
  const scan = (dir: string) => {
    ...
    for (const ent of fs.readdirSync(path.join(root, dir), { withFileTypes: true })) {
      ...
      if (!ent.isFile() || !/ka75|KA75/.test(rel) || !/\.(ts|lean|md|json)$/.test(ent.name)) continue;
  ...
  return { antiSpaghettiGatePassed: oversized.length === 0 && toolFiles.length <= 2 && !semanticPackageTouched, ka75Files: files, ka75ToolFiles: toolFiles, newKA75OversizedFiles: oversized, semanticPackageTouched, forbiddenPrefixes };
}
...
function checkLean() {
  ...
  if (!exists(relLean)) return { status: 'failed', reason: 'ka75_lean_module_missing', modules: [] as any[] };
  ...
  if (build.status !== 0) return { status: 'failed', reason: 'lean4lean_infertype_build_failed', modules: [{ ka: 'KA75-dependency-build', status: 'failed', proofBearing: false, obligations: 0, stdoutTail: (build.stdout ?? '').slice(-1800), stderrTail: (build.stderr ?? '').slice(-1800) }] };
  ...
  return { status: r.status === 0 ? 'passed' : 'failed', reason: r.status === 0 ? null : 'ka75_lean_module_failed', modules: [{ ka: 'KA75', source: relLean, destination: destLean, status: r.status === 0 ? 'passed' : 'failed', proofBearing: true, obligations: NEW_OBLIGATIONS, dependencyBuildStatus: 'passed', stdoutTail: (r.stdout ?? '').slice(-1800), stderrTail: (r.stderr ?? '').slice(-1800) }] };
}

function progress() {
  return { checkpoint: CHECKPOINT, publicVersion: VERSION, featureSurfaceBridgeProgressPercent: FEATURE_PROGRESS, executableKernelEquivalenceProofProgressPercent: EXECUTABLE_PROGRESS, arenaCorpusRegressionPercent: 100, formalLean4LeanBridgeObligations: TOTAL_OBLIGATIONS, obligationAccounting: { previousFormalLean4LeanBridgeObligations: BASELINE_OBLIGATIONS, newFormalLean4LeanBridgeObligations: NEW_OBLIGATIONS, ledgerCorrectionFormalLean4LeanBridgeObligations: 0, countedNewObligations: counted }, metricPolicy: 'KA75 starts the InferType proof-bearing surface with atomic level/free-variable/environment/sort facts; percentages are conservative dashboard estimates and not equivalence theorems.', notAFormalEquivalenceClaim: true };
}

function writeReports(result: any) {
  writeJson('assurance/ka75/KA75_EXECUTABLE_INFER_ATOMIC_REFINEMENT_RELEASE_GATE.json', result.releaseGate);
  writeJson('assurance/ka75/KA75_EXECUTABLE_INFER_ATOMIC_REFINEMENT_VERIFICATION_SUMMARY.json', result.verificationSummary);
  writeJson('assurance/ka75/KA75_KERNEL_FEATURE_EQUIVALENCE_PROGRESS.json', result.featureEquivalenceProgress);
  writeJson('assurance/ka75/KA75_EXECUTABLE_INFER_ATOMIC_REFINEMENT_BRIDGE_SPEC.json', result.bridgeSpec);
  fs.writeFileSync(path.join(root, 'assurance/ka75/KA75_KERNEL_FEATURE_EQUIVALENCE_PROGRESS_REPORT.md'), `# KA-75 Kernel Feature Equivalence Progress\n\nCheckpoint: \`${CHECKPOINT}\`\n\nPublic version: \`${VERSION}\`\n\n- Feature-surface bridge progress: **${FEATURE_PROGRESS}%** conservative dashboard estimate\n- Executable-kernel equivalence proof progress: **${EXECUTABLE_PROGRESS}%** conservative dashboard estimate\n- Arena corpus regression evidence: **100%**\n- Formal Lean4Lean bridge obligations: **${TOTAL_OBLIGATIONS}**\n\nKA-75 adds the first strict InferType atomic bridge slice after the KA-73/KA-74 DefEq ledger correction.\n`);
  fs.writeFileSync(path.join(root, 'assurance/ka75/KA75_EXECUTABLE_INFER_ATOMIC_REFINEMENT_REPORT.md'), `# KA-75 Executable Infer Atomic Refinement Report\n\nCheckpoint: \`${CHECKPOINT}\`\n\nPublic version: \`${VERSION}\`\n\nBaseline: \`${BASELINE}\`\n\n## What changed\n\nKA-75 adds four strict Lean4Lean bridge wrappers for atomic executable inference facts: level checking, free-variable inference, environment lookup, and sort typing. These wrappers are source-backed by Lean4Lean and are not \`sorry\`-backed. No trusted PSKernel semantics, codec, Core format, or certificate format are changed.\n\n## Machine-checked bridge lemmas\n\n${counted.map(x => `- \`${x}\``).join('\n')}\n\n## No-spaghetti result\n\n- Anti-spaghetti gate: **${result.architectureHealth.antiSpaghettiGatePassed ? 'passed' : 'failed'}**\n- KA-75 tool files: **${result.architectureHealth.ka75ToolFiles.length}**\n- New KA-75 oversized files: **${result.architectureHealth.newKA75OversizedFiles.length}**\n- Semantic package touched: **${result.architectureHealth.semanticPackageTouched}**\n\n## Boundary\n\n- Full Lean4 equivalence: **no**\n- Same theory as full Lean4: **no**\n- Fully formal K3: **no**\n- Executable PSKernel refinement proof: **no**\n- Full executable InferType refinement: **no**\n- Trusted PSKernel semantic change: **no**\n- Core format changed: **no**\n- Certificate format changed: **no**\n`);
}

export function runKA75(options: { strict?: boolean; writeReports?: boolean } = {}) {
  const pkg = readJson('package.json'); assert.equal(pkg.version, VERSION);
  const baselineProgress = readJson('assurance/ka74/KA74_KERNEL_FEATURE_EQUIVALENCE_PROGRESS.json');
  assert.equal(baselineProgress.formalLean4LeanBridgeObligations, BASELINE_OBLIGATIONS, 'KA74 obligation count must be the KA75 baseline');
  const lean = checkLean(), arch = architectureHealth(), prog = progress();
  const passed = lean.status === 'passed' && arch.antiSpaghettiGatePassed;
  const bridgeSpec = { checkpoint: CHECKPOINT, publicVersion: VERSION, baseline: BASELINE, referenceKind: 'strict-lean4lean-executable-infer-atomic-refinement', coreFormat: CORE_FORMAT, certificateFormat: CERT_FORMAT, leanModule: relLean, baselineFormalObligations: BASELINE_OBLIGATIONS, countedNewFormalObligations: NEW_OBLIGATIONS, countedObligations: counted, sourceTheorems, sorryBoundaryPolicy: 'do-not-count-upstream-sorry-backed-theorems', claimBoundary };
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const result = runKA75({ strict: process.argv.includes('--strict'), writeReports: true });
