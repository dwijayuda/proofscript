...
const CHECKPOINT = 'proofscript-v1-ka58-executable-expression-translator-invariant-refinement0';
const VERSION = '1.0.0-pskernel.61';
const BASELINE = 'proofscript-v1-ka57-executable-expression-translator-source-transform-refinement0';
const CORE_FORMAT = 71, CERT_FORMAT = 2;
const PREVIOUS_OBLIGATIONS = 163, NEW_OBLIGATIONS = 4, TOTAL_OBLIGATIONS = 167;
...
const relLean = 'assurance/ka58/executable-expression-translator-invariant-refinement-bridge.lean';
const destLean = 'PSKernelKA58ExecutableExpressionTranslatorInvariantRefinementBridge.lean';
const counted = [
...
function architectureHealth() {
  ...
  const scan = (dir: string) => {
    ...
    for (const ent of fs.readdirSync(path.join(root, dir), { withFileTypes: true })) {
      ...
      if (!ent.isFile() || !/ka58|KA58/.test(rel) || !/\.(ts|lean|md|json)$/.test(ent.name)) continue;
  ...
  return { antiSpaghettiGatePassed: oversized.length === 0 && toolFiles.length <= 2 && !semanticPackageTouched, ka58Files: files, ka58ToolFiles: toolFiles, newKA58OversizedFiles: oversized, semanticPackageTouched, forbiddenPrefixes };
}
...
function checkLean() {
  ...
  if (!exists(relLean)) return { status: 'failed', reason: 'ka58_lean_module_missing', modules: [] as any[] };
  ...
  return { status: r.status === 0 ? 'passed' : 'failed', reason: r.status === 0 ? null : 'ka58_lean_module_failed', modules: [{ ka: 'KA58', source: relLean, destination: destLean, status: r.status === 0 ? 'passed' : 'failed', proofBearing: true, obligations: NEW_OBLIGATIONS, stdoutTail: (r.stdout ?? '').slice(-1200), stderrTail: (r.stderr ?? '').slice(-1200) }] };
}

function progress() {
  return { checkpoint: CHECKPOINT, publicVersion: VERSION, featureSurfaceBridgeProgressPercent: 80, executableKernelEquivalenceProofProgressPercent: 45, arenaCorpusRegressionPercent: 100, formalLean4LeanBridgeObligations: TOTAL_OBLIGATIONS, obligationAccounting: { previousFormalLean4LeanBridgeObligations: PREVIOUS_OBLIGATIONS, newFormalLean4LeanBridgeObligations: NEW_OBLIGATIONS, countedNewObligations: counted }, groups: [ { id: 'expression-tags', progressPercent: 90, status: 'TrExprS constructors/determinism/substitution/source conditions/source transforms/source invariants strict-checked; full executable translator refinement incomplete' }, { id: 'typechecker-whnf-defeq', progressPercent: 53, status: 'WHNF/DefEq surface strict-checked; executable checker refinement incomplete' }, { id: 'end-to-end-refinement-spine', progressPercent: 35, status: 'checker pipeline skeleton strict-checked; no end-to-end theorem yet' }, { id: 'architecture-health', progressPercent: 93, status: 'no-spaghetti ratchet plus KA58 semantic-package touch guard' } ], notAFormalEquivalenceClaim: true };
}

function writeReports(result: any) {
  writeJson('assurance/ka58/KA58_EXECUTABLE_EXPRESSION_TRANSLATOR_INVARIANT_REFINEMENT_RELEASE_GATE.json', result.releaseGate);
  writeJson('assurance/ka58/KA58_EXECUTABLE_EXPRESSION_TRANSLATOR_INVARIANT_REFINEMENT_VERIFICATION_SUMMARY.json', result.verificationSummary);
  writeJson('assurance/ka58/KA58_KERNEL_FEATURE_EQUIVALENCE_PROGRESS.json', result.featureEquivalenceProgress);
  writeJson('assurance/ka58/executable-expression-translator-invariant-refinement-bridge-spec.json', result.bridgeSpec);
  fs.writeFileSync(path.join(root, 'assurance/ka58/KA58_KERNEL_FEATURE_EQUIVALENCE_PROGRESS_REPORT.md'), `# KA-58 Kernel Feature Equivalence Progress\n\nCheckpoint: \`${CHECKPOINT}\`\n\nPublic version: \`${VERSION}\`\n\n- Feature-surface bridge progress: **80%**\n- Executable-kernel equivalence proof progress: **45%**\n- Arena corpus regression evidence: **100%**\n- Formal Lean4Lean bridge obligations: **${TOTAL_OBLIGATIONS}**\n\nKA-58 counts four strict Lean4Lean obligations for source expression translation invariants used by executable expression translation. It does not claim full executable expression translator refinement.\n`);
  fs.writeFileSync(path.join(root, 'assurance/ka58/KA58_EXECUTABLE_EXPRESSION_TRANSLATOR_INVARIANT_REFINEMENT_REPORT.md'), `# KA-58 Executable Expression Translator Invariant Refinement Report\n\nCheckpoint: \`${CHECKPOINT}\`\n\nPublic version: \`${VERSION}\`\n\nBaseline: \`${BASELINE}\`\n\n## What changed\n\nKA-58 promotes Lean4Lean expression translation source invariant facts into explicit ProofScript/PSKernel bridge wrappers. It covers TrExprS closedness, TrExprS free-variable membership, TrExpr closedness, and TrExpr free-variable membership. It still does not prove full executable expression translator refinement.\n\n## Machine-checked bridge lemmas\n\n- \`translated_trExprS_closed_wf\`\n- \`translated_trExprS_fvarsIn_wf\`\n- \`translated_trExpr_closed_wf\`\n- \`translated_trExpr_fvarsIn_wf\`\n\n## No-spaghetti result\n\n- Anti-spaghetti gate: **${result.architectureHealth.antiSpaghettiGatePassed ? 'passed' : 'failed'}**\n- KA-58 tool files: **${result.architectureHealth.ka58ToolFiles.length}**\n- New KA-58 oversized files: **${result.architectureHealth.newKA58OversizedFiles.length}**\n- Semantic package touched: **${result.architectureHealth.semanticPackageTouched}**\n\n## Boundary\n\n- Full Lean4 equivalence: **no**\n- Same theory as full Lean4: **no**\n- Fully formal K3: **no**\n- Executable PSKernel refinement proof: **no**\n- Full executable expression translator refinement: **no**\n- Trusted PSKernel semantic change: **no**\n- Core format changed: **no**\n- Certificate format changed: **no**\n`);
}

export function runKA58(options: { strict?: boolean; writeReports?: boolean } = {}) {
  ...
  const bridgeSpec = { checkpoint: CHECKPOINT, publicVersion: VERSION, baseline: BASELINE, referenceKind: 'strict-lean4lean-expression-translator-invariant-refinement', coreFormat: CORE_FORMAT, certificateFormat: CERT_FORMAT, leanModule: relLean, countedNewFormalObligations: NEW_OBLIGATIONS, countedNewObligationNames: counted, claimBoundary };
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const result = runKA58({ strict: process.argv.includes('--strict'), writeReports: true });
