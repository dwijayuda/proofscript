...
export function runKA21NonDefEnvWFBridgeGate(options: { strict?: boolean; soft?: boolean } = {}) {
  for (const rel of [
  ...
  const spec = readJson('assurance/ka21/nondef-env-wf-bridge.json');
  const delta = readJson('assurance/ka21/obligation-delta.json');

  assert.match(pkg.version, /^1\.0\.0-pskernel\.(2[4-9]|[3-9][0-9]+)$/);
  ...
  assert.ok(typeof versions.latestLocalLineageCheckpoint === 'string');
  assert.equal(versions.ka21Checkpoint, 'proofscript-v1-ka21-nondef-env-wf-bridge0');
  assert.equal(versions.ka21TrustedSemanticChange, false);
  assert.equal(versions.ka21KernelCodecChange, false);
  assert.equal(versions.ka21NewTrustedComputationRule, false);
  assert.equal(versions.ka21FormalLean4EquivalenceProvenObligations, 22);
  assert.ok(versions.formalLean4EquivalenceProvenObligations >= 22);
  assert.equal(spec.referenceKind, 'direct-imported-lean4lean-nondef-env-wf-bridge');
  ...
  assert.equal(spec.claimBoundary.formalLean4EquivalenceProvenObligations, 22);
  assert.equal(delta.formalLean4EquivalenceProvenObligations, 22);

  const ka20Gate = readJson('assurance/ka20/KA20_RELEASE_GATE.json');
  assert.equal(ka20Gate.checkpoint, 'proofscript-v1-ka20-nondef-vdecl-wf-bridge0');
  assert.equal(ka20Gate.directLean4LeanNonDefVDeclWFBridge?.directLean4LeanNonDefVDeclWFBridgeChecked, true);
  ...
  const bridgeCheck = run(materialized.lakePath, ['env', materialized.leanPath, '-R', '.', path.join(lean4leanRoot, 'PSKernelKA21NonDefEnvWFBridge.lean')], { cwd: lean4leanRoot, env });

  const blockedReasons: string[] = [];
  if (ka20Gate.directLean4LeanNonDefVDeclWFBridge?.directLean4LeanNonDefVDeclWFBridgeChecked !== true) blockedReasons.push('ka20_nondef_vdecl_wf_bridge_not_passed');
  ...
  if (bridgeCheck.status !== 0) blockedReasons.push('ka21_nondef_env_wf_bridge_check_failed');
  ...
  const result = {
    ...
    ka20GateCheckpoint: ka20Gate.checkpoint,
    ka20GatePassed: ka20Gate.directLean4LeanNonDefVDeclWFBridge?.directLean4LeanNonDefVDeclWFBridgeChecked === true,
    ...
    directLean4LeanNonDefEnvWFBridgeChecked: strictBridgeCheckPassed,
    ...
    fullyFormalK3: false,
    formalLean4EquivalenceProvenObligations: 22,
  };

  if (options.strict && !strictBridgeCheckPassed) {
    const err = new Error(`KA-21 strict nondef VEnv.WF bridge blocked: ${blockedReasons.join(', ')}`);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  try {
    const result = runKA21NonDefEnvWFBridgeGate({ strict: process.argv.includes('--strict'), soft: process.argv.includes('--soft') });
