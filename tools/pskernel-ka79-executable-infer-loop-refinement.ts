...
const CHECKPOINT = 'proofscript-v1-ka79-executable-infer-loop-refinement0';
const VERSION = '1.0.0-pskernel.82';
const BASELINE = 'proofscript-v1-ka78-recursor-projection-sorry-boundary-audit0';
const CORE_FORMAT = 71;
const CERT_FORMAT = 2;
const BASELINE_OBLIGATIONS = 232;
...
const relLean = 'assurance/ka79/executable-infer-loop-refinement-bridge.lean';
const destLean = 'PSKernelKA79ExecutableInferLoopRefinementBridge.lean';
const counted = [
...
function architectureHealth() {
  ...
  const scan = (dir: string) => {
    ...
    for (const ent of fs.readdirSync(abs(dir), { withFileTypes: true })) {
      ...
      if (!ent.isFile() || !/ka79|KA79/.test(rel) || !/\.(ts|lean|md|json)$/.test(ent.name)) continue;
  ...
  return { antiSpaghettiGatePassed: oversized.length === 0 && toolFiles.length <= 2 && !semanticPackageTouched, ka79Files: files, ka79ToolFiles: toolFiles, newKA79OversizedFiles: oversized, semanticPackageTouched, forbiddenPrefixes };
}
function checkLean() {
  ...
  if (!exists(relLean)) return { status: 'failed', reason: 'ka79_lean_module_missing', modules: [] as any[] };
  ...
  if (build.status !== 0) return { status: 'failed', reason: 'lean4lean_infertype_build_failed', modules: [{ ka: 'KA79-dependency-build', status: 'failed', proofBearing: false, obligations: 0, stdoutTail: (build.stdout ?? '').slice(-1800), stderrTail: (build.stderr ?? '').slice(-1800) }] };
  fs.copyFileSync(abs(relLean), path.join(lean4leanRoot, destLean));
  const r = run(lakePath, ['env', leanPath, destLean], lean4leanRoot);
  return { status: r.status === 0 ? 'passed' : 'failed', reason: r.status === 0 ? null : 'ka79_lean_module_failed', modules: [{ ka: 'KA79', source: relLean, destination: destLean, status: r.status === 0 ? 'passed' : 'failed', proofBearing: true, obligations: NEW_OBLIGATIONS, dependencyBuildStatus: 'passed', stdoutTail: (r.stdout ?? '').slice(-1800), stderrTail: (r.stderr ?? '').slice(-1800) }] };
}
function progress() {
  return { checkpoint: CHECKPOINT, publicVersion: VERSION, featureSurfaceBridgeProgressPercent: FEATURE_PROGRESS, executableKernelEquivalenceProofProgressPercent: EXECUTABLE_PROGRESS, arenaCorpusRegressionPercent: 100, formalLean4LeanBridgeObligations: TOTAL_OBLIGATIONS, obligationAccounting: { previousFormalLean4LeanBridgeObligations: BASELINE_OBLIGATIONS, newFormalLean4LeanBridgeObligations: NEW_OBLIGATIONS, ledgerCorrectionFormalLean4LeanBridgeObligations: 0, correctedFormalLean4LeanBridgeObligations: TOTAL_OBLIGATIONS, countedNewObligations: counted }, metricPolicy: 'KA79 adds direct non-sorry-backed InferType loop bridge wrappers. Percentages remain conservative dashboard estimates, not equivalence theorems.', notAFormalEquivalenceClaim: true };
}
function writeReports(result: any) {
  const dir = 'assurance/ka79';
  writeJson(`${dir}/KA79_EXECUTABLE_INFER_LOOP_REFINEMENT_RELEASE_GATE.json`, result.releaseGate);
  writeJson(`${dir}/KA79_EXECUTABLE_INFER_LOOP_REFINEMENT_VERIFICATION_SUMMARY.json`, result.verificationSummary);
  writeJson(`${dir}/KA79_KERNEL_FEATURE_EQUIVALENCE_PROGRESS.json`, result.featureEquivalenceProgress);
  writeJson(`${dir}/KA79_EXECUTABLE_INFER_LOOP_REFINEMENT_BRIDGE_SPEC.json`, result.bridgeSpec);
  fs.writeFileSync(abs(`${dir}/KA79_KERNEL_FEATURE_EQUIVALENCE_PROGRESS_REPORT.md`), `# KA-79 Kernel Feature Equivalence Progress\n\nCheckpoint: \`${CHECKPOINT}\`\n\n- Feature-surface bridge progress: **${FEATURE_PROGRESS}%** conservative dashboard estimate\n- Executable-kernel equivalence proof progress: **${EXECUTABLE_PROGRESS}%** conservative dashboard estimate\n- Arena corpus regression evidence: **100%**\n- Formal Lean4Lean bridge obligations: **${TOTAL_OBLIGATIONS}**\n\nThese are not full Lean 4 equivalence claims.\n`);
  fs.writeFileSync(abs(`${dir}/KA79_EXECUTABLE_INFER_LOOP_REFINEMENT_REPORT.md`), `# KA-79 Executable Infer Loop Refinement\n\nCheckpoint: \`${CHECKPOINT}\`\n\nPublic version: \`${VERSION}\`\n\nBaseline: \`${BASELINE}\`\n\nKA-79 adds ${NEW_OBLIGATIONS} strict Lean4Lean bridge obligations over direct non-sorry-backed InferType loop support theorems. Total formal Lean4Lean bridge obligations: **${TOTAL_OBLIGATIONS}**.\n\n## Counted obligations\n\n${counted.map(x => `- \`${x}\``).join('\n')}\n\n## Boundary\n\nNo full Lean4 equivalence, no executable PSKernel refinement proof, no trusted semantic package change, no Core format change, and no certificate format change.\n`);
}
function main() {
  const pkg = readJson('package.json');
  assert.equal(pkg.version, VERSION, 'package version must match KA79 public version');
  const base = readJson('assurance/ka78/KA78_KERNEL_FEATURE_EQUIVALENCE_PROGRESS.json');
  assert.equal(base.formalLean4LeanBridgeObligations, BASELINE_OBLIGATIONS, 'KA78 baseline obligations must be 232');
  const lean = checkLean();
  assert.equal(lean.status, 'passed', `KA79 strict Lean check must pass: ${JSON.stringify(lean)}`);
  const arch = architectureHealth();
  assert.equal(arch.antiSpaghettiGatePassed, true, 'KA79 anti-spaghetti gate must pass');
  const featureEquivalenceProgress = progress();
  const bridgeSpec = { checkpoint: CHECKPOINT, publicVersion: VERSION, baseline: BASELINE, referenceKind: 'executable-infer-loop-refinement', coreFormat: CORE_FORMAT, certificateFormat: CERT_FORMAT, countedFormalObligations: NEW_OBLIGATIONS, countedObligations: counted, sourceTheorems, directSorryPolicy: 'do-not-count-upstream-theorems-whose-own-body-is-sorry', claimBoundary };
  const releaseGate = { checkpoint: CHECKPOINT, publicVersion: VERSION, baseline: BASELINE, releaseKind: 'strict-lean4lean-infer-loop-refinement', kernel: { activeKernel: 'PSKernel', coreFormat: CORE_FORMAT, certificateFormat: CERT_FORMAT }, strictLean4LeanStatus: lean.status, strictLean4LeanModules: lean.modules, featureEquivalenceProgress, architectureHealth: arch, claimBoundary, trustedSemanticPackageChange: false, coreFormatChanged: false, certificateFormatChanged: false };
  const verificationSummary = { checkpoint: CHECKPOINT, publicVersion: VERSION, baseline: BASELINE, focusedGateStatus: 'passed', strictLean4LeanStatus: lean.status, formalLean4LeanBridgeObligations: TOTAL_OBLIGATIONS, newFormalLean4LeanBridgeObligations: NEW_OBLIGATIONS, baselineObligations: BASELINE_OBLIGATIONS, countedObligations: counted, redTestObserved: true, redTestReason: 'KA79 test failed first on missing KA79 gate tool', noWrapperTimeoutCountedAsPassed: true };
