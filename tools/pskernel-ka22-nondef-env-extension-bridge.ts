...
export function runKA22NonDefEnvExtensionBridgeGate(options: { strict?: boolean; soft?: boolean } = {}) {
  for (const rel of [
  ...
  const spec = readJson('assurance/ka22/nondef-env-extension-bridge.json');
  const delta = readJson('assurance/ka22/obligation-delta.json');

  assert.match(pkg.version, /^1\.0\.0-pskernel\.(25|26|27)$/);
  ...
  assert.match(versions.latestLocalLineageCheckpoint, /^proofscript-v1-ka(22-nondef-env-extension|23-nondef-env-lookup|24-nondef-env-no-overwrite)-bridge0$/);
  assert.equal(versions.ka22Checkpoint, 'proofscript-v1-ka22-nondef-env-extension-bridge0');
  assert.equal(versions.ka22TrustedSemanticChange, false);
  assert.equal(versions.ka22KernelCodecChange, false);
  assert.equal(versions.ka22NewTrustedComputationRule, false);
  assert.equal(versions.ka22FormalLean4EquivalenceProvenObligations, 25);
  assert.ok(versions.formalLean4EquivalenceProvenObligations >= 25);
  assert.equal(spec.referenceKind, 'direct-imported-lean4lean-nondef-env-extension-bridge');
  ...
  assert.equal(spec.claimBoundary.formalLean4EquivalenceProvenObligations, 25);
  assert.equal(delta.formalLean4EquivalenceProvenObligations, 25);

  const ka21Gate = readJson('assurance/ka21/KA21_RELEASE_GATE.json');
  assert.equal(ka21Gate.checkpoint, 'proofscript-v1-ka21-nondef-env-wf-bridge0');
  assert.equal(ka21Gate.directLean4LeanNonDefEnvWFBridge?.directLean4LeanNonDefEnvWFBridgeChecked, true);
  ...
  const bridgeCheck = run(materialized.lakePath, ['env', materialized.leanPath, '-R', '.', path.join(lean4leanRoot, 'PSKernelKA22NonDefEnvExtensionBridge.lean')], { cwd: lean4leanRoot, env });

  const blockedReasons: string[] = [];
  if (ka21Gate.directLean4LeanNonDefEnvWFBridge?.directLean4LeanNonDefEnvWFBridgeChecked !== true) blockedReasons.push('ka21_nondef_env_wf_bridge_not_passed');
  ...
  if (bridgeCheck.status !== 0) blockedReasons.push('ka22_nondef_env_extension_bridge_check_failed');
  ...
  const result = {
    ...
    ka21GateCheckpoint: ka21Gate.checkpoint,
    ka21GatePassed: ka21Gate.directLean4LeanNonDefEnvWFBridge?.directLean4LeanNonDefEnvWFBridgeChecked === true,
    ...
    directLean4LeanNonDefEnvExtensionBridgeChecked: strictBridgeCheckPassed,
    ...
    fullyFormalK3: false,
    formalLean4EquivalenceProvenObligations: 25,
  };

  if (options.strict && !strictBridgeCheckPassed) {
    const err = new Error(`KA-22 strict nondef VEnv.LE bridge blocked: ${blockedReasons.join(', ')}`);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  try {
    const result = runKA22NonDefEnvExtensionBridgeGate({ strict: process.argv.includes('--strict'), soft: process.argv.includes('--soft') });
