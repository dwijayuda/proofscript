...
for (const supported of ["textDocument/hover","textDocument/documentSymbol","proofscript/documentStatus","proofscript/semanticInfo"]) {
  assert.ok(extension.includes(supported), "missing active editor method " + supported);
}
for (const unsupported of ["textDocument/completion","textDocument/definition","textDocument/references","textDocument/rename","proofscript/proofState"]) {
