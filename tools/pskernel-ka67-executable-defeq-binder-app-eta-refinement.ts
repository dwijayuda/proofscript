...
const CHECKPOINT = 'proofscript-v1-ka67-executable-defeq-binder-app-eta-refinement0';
const VERSION = '1.0.0-pskernel.70';
const BASELINE = 'proofscript-v1-ka66-executable-defeq-core-refinement0';
const CORE_FORMAT = 71, CERT_FORMAT = 2;
const BASELINE_OBLIGATIONS = 200, NEW_OBLIGATIONS = 4, TOTAL_OBLIGATIONS = 204;
const FEATURE_PROGRESS = 87, EXECUTABLE_PROGRESS = 53;
const OLD_CODEBASE_POLICY = { oldCodebaseRegressionIgnoredByUser: true, fullNpmTestRequiredForThisCheckpoint: false, reason: 'User explicitly requested continuing with KA66 and ignoring old codebase; KA67 changes only strict Lean4Lean bridge wrappers and release metadata.' };
...
const relLean = 'assurance/ka67/executable-defeq-binder-app-eta-refinement-bridge.lean';
const destLean = 'PSKernelKA67ExecutableDefEqBinderAppEtaRefinementBridge.lean';
const counted = [
...
function architectureHealth() {
  ...
  const scan = (dir: string) => {
    ...
    for (const ent of fs.readdirSync(path.join(root, dir), { withFileTypes: true })) {
      ...
      if (!ent.isFile() || !/ka67|KA67/.test(rel) || !/\.(ts|lean|md|json)$/.test(ent.name)) continue;
  ...
  return { antiSpaghettiGatePassed: oversized.length === 0 && toolFiles.length <= 2 && !semanticPackageTouched, ka67Files: files, ka67ToolFiles: toolFiles, newKA67OversizedFiles: oversized, semanticPackageTouched, forbiddenPrefixes };
}
...
function checkLean() {
  ...
  if (!exists(relLean)) return { status: 'failed', reason: 'ka67_lean_module_missing', modules: [] as any[] };
  const build = run(lakePath, ['build', 'Lean4Lean.Verify.TypeChecker.IsDefEq'], lean4leanRoot);
  if (build.status !== 0) return { status: 'failed', reason: 'lean4lean_isdefeq_build_failed', modules: [{ ka: 'KA67-dependency-build', source: 'Lean4Lean.Verify.TypeChecker.IsDefEq', status: 'failed', proofBearing: false, obligations: 0, stdoutTail: (build.stdout ?? '').slice(-1800), stderrTail: (build.stderr ?? '').slice(-1800) }] };
  ...
  return { status: r.status === 0 ? 'passed' : 'failed', reason: r.status === 0 ? null : 'ka67_lean_module_failed', modules: [{ ka: 'KA67', source: relLean, destination: destLean, status: r.status === 0 ? 'passed' : 'failed', proofBearing: true, obligations: NEW_OBLIGATIONS, dependencyBuildStatus: 'passed', stdoutTail: (r.stdout ?? '').slice(-1800), stderrTail: (r.stderr ?? '').slice(-1800) }] };
}

function progress() {
  return { checkpoint: CHECKPOINT, publicVersion: VERSION, featureSurfaceBridgeProgressPercent: FEATURE_PROGRESS, executableKernelEquivalenceProofProgressPercent: EXECUTABLE_PROGRESS, arenaCorpusRegressionPercent: 100, formalLean4LeanBridgeObligations: TOTAL_OBLIGATIONS, obligationAccounting: { previousFormalLean4LeanBridgeObligations: BASELINE_OBLIGATIONS, newFormalLean4LeanBridgeObligations: NEW_OBLIGATIONS, countedNewObligations: counted }, metricPolicy: 'KA67 advances the exact formal obligation counter only; percentage estimates remain held at the latest audited KA65/KA66 values pending a separate coverage audit.', notAFormalEquivalenceClaim: true };
}

function writeReports(result: any) {
  writeJson('assurance/ka67/KA67_EXECUTABLE_DEFEQ_BINDER_APP_ETA_REFINEMENT_RELEASE_GATE.json', result.releaseGate);
  writeJson('assurance/ka67/KA67_EXECUTABLE_DEFEQ_BINDER_APP_ETA_REFINEMENT_VERIFICATION_SUMMARY.json', result.verificationSummary);
  writeJson('assurance/ka67/KA67_KERNEL_FEATURE_EQUIVALENCE_PROGRESS.json', result.featureEquivalenceProgress);
  writeJson('assurance/ka67/executable-defeq-binder-app-eta-refinement-bridge-spec.json', result.bridgeSpec);
  fs.writeFileSync(path.join(root, 'assurance/ka67/KA67_KERNEL_FEATURE_EQUIVALENCE_PROGRESS_REPORT.md'), `# KA-67 Kernel Feature Equivalence Progress\n\nCheckpoint: \`${CHECKPOINT}\`\n\nPublic version: \`${VERSION}\`\n\n- Feature-surface bridge progress: **${FEATURE_PROGRESS}%** (held pending audit)\n- Executable-kernel equivalence proof progress: **${EXECUTABLE_PROGRESS}%** (held pending audit)\n- Arena corpus regression evidence: **100%**\n- Formal Lean4Lean bridge obligations: **${TOTAL_OBLIGATIONS}**\n\nKA-67 adds four strict DefEq binder/application/eta bridge obligations. Percentage estimates are intentionally held without a separate coverage audit.\n`);
  fs.writeFileSync(path.join(root, 'assurance/ka67/KA67_EXECUTABLE_DEFEQ_BINDER_APP_ETA_REFINEMENT_REPORT.md'), `# KA-67 Executable DefEq Binder/App/Eta Refinement Report\n\nCheckpoint: \`${CHECKPOINT}\`\n\nPublic version: \`${VERSION}\`\n\nBaseline: \`${BASELINE}\`\n\n## What changed\n\nKA-67 continues from KA-66 and adds four strict Lean4Lean DefEq bridge wrappers for lambda binders, forall binders, application DefEq, and eta expansion. No trusted PSKernel semantics, codec, Core format, or certificate format are changed. Per user instruction, the old-codebase indexed-recursor regression is ignored for this continuation; KA-67 is verified through bounded KA/release gates rather than the repository-wide legacy suite.\n\n## Machine-checked bridge lemmas\n\n${counted.map(x => `- \`${x}\``).join('\n')}\n\n## No-spaghetti result\n\n- Anti-spaghetti gate: **${result.architectureHealth.antiSpaghettiGatePassed ? 'passed' : 'failed'}**\n- KA-67 tool files: **${result.architectureHealth.ka67ToolFiles.length}**\n- New KA-67 oversized files: **${result.architectureHealth.newKA67OversizedFiles.length}**\n- Semantic package touched: **${result.architectureHealth.semanticPackageTouched}**\n\n## Boundary\n\n- Full Lean4 equivalence: **no**\n- Same theory as full Lean4: **no**\n- Fully formal K3: **no**\n- Executable PSKernel refinement proof: **no**\n- Full executable WHNF/DefEq refinement: **no**\n- Trusted PSKernel semantic change: **no**\n- Core format changed: **no**\n- Certificate format changed: **no**\n`);
}

export function runKA67(options: { strict?: boolean; writeReports?: boolean } = {}) {
  const pkg = readJson('package.json'); assert.equal(pkg.version, VERSION);
  const ka66 = readJson('assurance/ka66/KA66_EXECUTABLE_DEFEQ_CORE_REFINEMENT_RELEASE_GATE.json');
  assert.equal(ka66.featureEquivalenceProgress.formalLean4LeanBridgeObligations, BASELINE_OBLIGATIONS, 'KA66 release gate must support the baseline');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const result = runKA67({ strict: process.argv.includes('--strict'), writeReports: true });
