...
const CHECKPOINT = 'proofscript-v1-ka63-executable-expression-translator-bool-literal-refinement0';
const VERSION = '1.0.0-pskernel.66';
const BASELINE = 'proofscript-v1-ka62-executable-expression-translator-bool-literal-refinement0';
const CORE_FORMAT = 71, CERT_FORMAT = 2;
const PREVIOUS_OBLIGATIONS = 184, NEW_OBLIGATIONS = 4, TOTAL_OBLIGATIONS = 188;
...
const relLean = 'assurance/ka63/executable-expression-translator-bool-literal-refinement-bridge.lean';
const destLean = 'PSKernelKA63ExecutableExpressionTranslatorBoolLiteralRefinementBridge.lean';
const counted = [
...
function architectureHealth() {
  ...
  const scan = (dir: string) => {
    ...
    for (const ent of fs.readdirSync(path.join(root, dir), { withFileTypes: true })) {
      ...
      if (!ent.isFile() || !/ka63|KA63/.test(rel) || !/\.(ts|lean|md|json)$/.test(ent.name)) continue;
  ...
  return { antiSpaghettiGatePassed: oversized.length === 0 && toolFiles.length <= 2 && !semanticPackageTouched, ka63Files: files, ka63ToolFiles: toolFiles, newKA63OversizedFiles: oversized, semanticPackageTouched, forbiddenPrefixes };
}
...
function checkLean() {
  ...
  if (!exists(relLean)) return { status: 'failed', reason: 'ka63_lean_module_missing', modules: [] as any[] };
  ...
  return { status: r.status === 0 ? 'passed' : 'failed', reason: r.status === 0 ? null : 'ka63_lean_module_failed', modules: [{ ka: 'KA63', source: relLean, destination: destLean, status: r.status === 0 ? 'passed' : 'failed', proofBearing: true, obligations: NEW_OBLIGATIONS, stdoutTail: (r.stdout ?? '').slice(-1200), stderrTail: (r.stderr ?? '').slice(-1200) }] };
}

function progress() {
  return { checkpoint: CHECKPOINT, publicVersion: VERSION, featureSurfaceBridgeProgressPercent: 85, executableKernelEquivalenceProofProgressPercent: 50, arenaCorpusRegressionPercent: 100, formalLean4LeanBridgeObligations: TOTAL_OBLIGATIONS, obligationAccounting: { previousFormalLean4LeanBridgeObligations: PREVIOUS_OBLIGATIONS, newFormalLean4LeanBridgeObligations: NEW_OBLIGATIONS, countedNewObligations: counted }, groups: [ { id: 'expression-tags', progressPercent: 96, status: 'TrExprS constructors/determinism/substitution/source conditions/source transforms/source invariants/application-list/beta/AppStack/uninstantiate/bool-literal facts strict-checked; full executable translator refinement incomplete' }, { id: 'typechecker-whnf-defeq', progressPercent: 53, status: 'WHNF/DefEq surface strict-checked; executable checker refinement incomplete' }, { id: 'end-to-end-refinement-spine', progressPercent: 35, status: 'checker pipeline skeleton strict-checked; no end-to-end theorem yet' }, { id: 'architecture-health', progressPercent: 95, status: 'no-spaghetti ratchet plus KA63 semantic-package touch guard' } ], notAFormalEquivalenceClaim: true };
}

function writeReports(result: any) {
  writeJson('assurance/ka63/KA63_EXECUTABLE_EXPRESSION_TRANSLATOR_BOOL_LITERAL_REFINEMENT_RELEASE_GATE.json', result.releaseGate);
  writeJson('assurance/ka63/KA63_EXECUTABLE_EXPRESSION_TRANSLATOR_BOOL_LITERAL_REFINEMENT_VERIFICATION_SUMMARY.json', result.verificationSummary);
  writeJson('assurance/ka63/KA63_KERNEL_FEATURE_EQUIVALENCE_PROGRESS.json', result.featureEquivalenceProgress);
  writeJson('assurance/ka63/executable-expression-translator-bool-literal-refinement-bridge-spec.json', result.bridgeSpec);
  fs.writeFileSync(path.join(root, 'assurance/ka63/KA63_KERNEL_FEATURE_EQUIVALENCE_PROGRESS_REPORT.md'), `# KA-62 Kernel Feature Equivalence Progress\n\nCheckpoint: \`${CHECKPOINT}\`\n\nPublic version: \`${VERSION}\`\n\n- Feature-surface bridge progress: **85%**\n- Executable-kernel equivalence proof progress: **50%**\n- Arena corpus regression evidence: **100%**\n- Formal Lean4Lean bridge obligations: **${TOTAL_OBLIGATIONS}**\n\nKA-63 counts four strict Lean4Lean obligations for Boolean literal translation and Boolean literal source free-variable closure facts used by executable expression translation. It does not claim full executable expression translator refinement.\n`);
  fs.writeFileSync(path.join(root, 'assurance/ka63/KA63_EXECUTABLE_EXPRESSION_TRANSLATOR_BOOL_LITERAL_REFINEMENT_REPORT.md'), `# KA-63 Executable Expression Translator Bool Literal Refinement Report\n\nCheckpoint: \`${CHECKPOINT}\`\n\nPublic version: \`${VERSION}\`\n\nBaseline: \`${BASELINE}\`\n\n## What changed\n\nKA-63 promotes Lean4Lean Boolean literal translation and Boolean literal source free-variable facts into explicit ProofScript/PSKernel bridge wrappers. It covers false, true, arbitrary Bool literal translation, and FVarsIn closure for Bool literals. It still does not prove full executable expression translator refinement.\n\n## Machine-checked bridge lemmas\n\n${counted.map(x => `- \`${x}\``).join('\n')}\n\n## No-spaghetti result\n\n- Anti-spaghetti gate: **${result.architectureHealth.antiSpaghettiGatePassed ? 'passed' : 'failed'}**\n- KA-63 tool files: **${result.architectureHealth.ka63ToolFiles.length}**\n- New KA-63 oversized files: **${result.architectureHealth.newKA63OversizedFiles.length}**\n- Semantic package touched: **${result.architectureHealth.semanticPackageTouched}**\n\n## Boundary\n\n- Full Lean4 equivalence: **no**\n- Same theory as full Lean4: **no**\n- Fully formal K3: **no**\n- Executable PSKernel refinement proof: **no**\n- Full executable expression translator refinement: **no**\n- Trusted PSKernel semantic change: **no**\n- Core format changed: **no**\n- Certificate format changed: **no**\n`);
}

export function runKA63(options: { strict?: boolean; writeReports?: boolean } = {}) {
  ...
  const bridgeSpec = { checkpoint: CHECKPOINT, publicVersion: VERSION, baseline: BASELINE, referenceKind: 'strict-lean4lean-expression-translator-bool-literal-refinement', coreFormat: CORE_FORMAT, certificateFormat: CERT_FORMAT, leanModule: relLean, countedNewFormalObligations: NEW_OBLIGATIONS, countedNewObligationNames: counted, claimBoundary };
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const result = runKA63({ strict: process.argv.includes('--strict'), writeReports: true });
