/-
PSKernel KA-4 Lean machine-check staging file.

This file is intentionally self-contained so a Lean 4.33.1 executable can
check the current non-inductive translation/soundness slice without requiring
Lean4Lean to be vendored yet. KA-4 adds the strict/soft proof-check gate; it
does not claim full Lean 4 equivalence.
-/

namespace PSKernelKA4
...
theorem unsupported_decl_kind_blocks_translation (d : PSDecl) :
    isNonInductive d.kind = false -> translateKind? d.kind = none := by
  intro h
  exact unsupported_kind_translation_blocked d.kind h
