...
- `tools/arena-tutorial-runner.ts` now resolves actual Arena tarball paths such as `good/tutorial/001_basicDef.ndjson` and `bad/tutorial/002_badDef.ndjson`.
- `tools/arena-tutorial-harness-tests.ts` covers nested Arena tutorial fixture layout.
...
- `tools/arena-checker-smoke.ts` now includes a regression fixture showing that `def`/`hints="opaque"` still unfolds for beta/delta checking.
...
node tools/arena-tutorial-runner.ts --fixtures-dir /mnt/data/arena-corpus-20260915: PASS, no wrong results
