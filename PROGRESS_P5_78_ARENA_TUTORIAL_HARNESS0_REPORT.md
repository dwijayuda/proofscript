...
- `tools/arena-tutorial-runner.mjs` runs any locally available Arena tutorial NDJSON fixtures through `packages/arena-checker/dist/main.js` and reports accepted-good, rejected-bad, declined-unsupported, wrong accepts, wrong rejects, checker crashes, and not-run counts.
- `tools/arena-tutorial-harness-tests.mjs` uses tiny hand-written NDJSON fixtures to verify the harness: one accepted good, one rejected bad, one declined unsupported, and one not-run missing fixture.
- Root scripts:
